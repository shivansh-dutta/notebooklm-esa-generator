...
This file needs to live somewhere Claude Code checks *before* the pipeline
repo is cloned — typically `~/.claude/skills/notebooklm-esa-intake/SKILL.md`
on each machine that will run this workflow (a project-local
`.claude/skills/` won't work here, since there's no project yet to be local
*to* the first time this runs in a fresh folder). Getting it distributed to
every machine (shared drive, MDM push, or a manual copy) is outside what
this skill can do for itself.
